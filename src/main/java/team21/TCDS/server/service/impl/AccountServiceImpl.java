...
import team21.TCDS.server.enumeration.VCLevel;
import team21.TCDS.server.pojo.ChangePasswordInfoPojo;
import team21.TCDS.server.pojo.LoginInfoPojo;
import team21.TCDS.server.pojo.PublicKeyInfo;
import team21.TCDS.server.pojo.SignUpInfoPojo;
import team21.TCDS.server.service.AccountService;
...
public class AccountServiceImpl implements AccountService {
	...
	public String checkLoginRequest(final HttpServletRequest request, final HttpSession session) {
		...
		try {
			...
			if (!ConfigureReader.instance().getVCLevel().equals(VCLevel.Close)) {
				synchronized (focusAccount) { //同步锁，将focusAccount加锁，范围为下面的代码块
	...
	public String changePassword(HttpServletRequest request) {
		...
		final String encrypted = request.getParameter("encrypted");
		try {
			final String changePasswordInfoStr = RSADecryptUtil.dncryption(encrypted, ku.getPrivateKey()); //解密
