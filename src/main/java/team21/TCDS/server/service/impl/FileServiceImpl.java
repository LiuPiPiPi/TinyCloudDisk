...
import team21.TCDS.server.listener.ServerInitListener;
...
import team21.TCDS.server.pojo.CheckImportFolderRespons;
import team21.TCDS.server.pojo.CheckUploadFilesRespons;
import team21.TCDS.server.service.FileService;
...
public class FileServiceImpl extends RangeFileStreamWriter implements FileService {
    ...
    public void downloadCheckedFilesZip(final HttpServletRequest request, final HttpServletResponse response)
        ...
        if (zipname != null && !zipname.equals("ERROR")) {
            ...
            String fname = "TCDS_" + ServerTimeUtil.accurateToDay() + "_\u6253\u5305\u4e0b\u8f7d.zip";
            if (zip.exists()) {
...
//				}
    ...
    public String confirmMoveFiles(HttpServletRequest request) {
        ...
        if (ConfigureReader.instance().accessFolder(targetFolder, account) && true)
//				ConfigureReader.instance().authorized(account, AccountAuth.MOVE_FILES,fu.getAllFoldersId(locationpath)))
        {
            try {
                final List<String> idList = gson.fromJson(strIdList, new TypeToken<List<String>>() {
                }.getType());
