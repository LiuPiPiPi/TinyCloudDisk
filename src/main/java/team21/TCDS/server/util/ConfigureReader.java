...
import team21.TCDS.server.enumeration.*;
import team21.TCDS.server.model.Folder;
import team21.TCDS.server.pojo.*;
 ...
 * <h2>TCDS配置解析器</h2>
 * <p>
 * 该工具负责读取并解析配置文件，并将结果随时提供给TCDS服务器业务逻辑以完成相应功能（例如用户认证、权限判定、配置启动端口等）。
 * </p>
 *
...
public class ConfigureReader {
    ...
    private TCDSProperties serverp;// 配置设置
    private TCDSProperties accountp;// 账户设置
    ...
    private final int DEFAULT_PORT = 8083; // 服务器端口 8080->8083
    ...
    private ConfigureReader() {
        ...
        this.serverp = new TCDSProperties();
        this.accountp = new TCDSProperties();
     ...
     * @param auth    team21.TCDS.server.enumeration.AccountAuth
...
//			}
     ...
     * </p>
     *
     * @return java.util.List<team21.TCDS.server.pojo.ExtendStores> 所有扩展存储区对象的列表
     * @author 高级软件21组(team21)
     */
     ...
     * <h2>获得验证码等级</h2>
     * <p>
     * 返回设置的验证码等级枚举类（team21.TCDS.server.enumeration.VCLevel），包括：关闭（CLOSE）、简单（Simplified）、标准（Standard）
     * </p>
     *
     * @return team21.TCDS.server.enumeration.VCLevel 验证码等级
     * @author 高级软件21组(team21)
     */
    ...
    private int testServerPropertiesAndEffect() {
        ...
        final String ports = null;
        if (ports == null) {
            Printer.instance.print("警告：未找到端口配置，将采用默认值（8080）。");
            this.port = 8083;
        } else {
            try {
                Printer.instance.print(ports);
            ...
            if ("true".equals(enableHttps)) {
                ...
                if (keyFile.isFile()) {
                    ...
                    if (keyFile.isFile()) {
                        ...
                        Printer.instance.print(
                                "错误：无法启用https支持，因为TCDS未能找到https证书文件。您必须在应用主目录内放置PKCS12（必须命名为https.p12）或JKS（必须命名为https.jks）证书。");
    ...
    public void createDefaultServerPropertiesFile() {
        ...
        try {
            dsp.store(new FileOutputStream(this.confdir + SERVER_PROPERTIES_FILE),
                    "<This is the default TCDS server setting file. >");
    ...
    private void createDefaultAccountPropertiesFile() {
        ...
        try (FileOutputStream accountSettingOut = new FileOutputStream(this.confdir + ACCOUNT_PROPERTIES_FILE)) {
            dap.store(accountSettingOut, "<This is the default TCDS account setting file. >");
        ...
        if (accountPropertiesUpdateDaemonThread == null) {
            ...
            accountPropertiesUpdateDaemonThread = new Thread(() -> {
                try {
                    ...
                    Printer.instance.print("错误：用户配置文件更改监听失败，该功能已失效，TCDS无法实时更新用户配置（可尝试重启程序以恢复该功能）。");
                }
            });
